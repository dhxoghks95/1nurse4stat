...
# 금융공학을 위한 R 기초 {#financial-R}

## 현재가치(Present Value) {#present-value}

[현재가치](https://ko.wikipedia.org/wiki/현재가치)는 현재가치와 장래가치로 구분되며, 
미래에 얻게 될 확실한 부(富)의 가치를 현재의 가치로 환산한 값을 말한다.
...
cash_df <- data.frame(household =c("갑돌", "갑돌", "갑돌"),
                      ...
                      # cash_flow =c(1000L,2000L, 3000L)으로 하면 integer 형태로 나옴
                      year = c(1,2,3)) 
cash_df

typeof(cash_df$household)
typeof(cash_df$cash_flow)
## 1년 후 현금흐름 계산(3%)
...
## 자료구조 {#financial-r-data-structure}
...
class(cash_df) #data.frame
```

### 요인(Factor) {#financial-r-factor}
...
## sample(c("유아", "청소년", "노인") ,3) 이런 식으로도 가능
eval_f <- cut(x = eval_v, breaks = c(0, 25, 50, 75, 100)) 
#factor를 만듦 여기선 0~25, 25~50, 50~75, 75~100의 네 구간으로 나뉨
levels(eval_f) <- c("매우 낮음", "낮음", "높음", "매우 높음")
table(eval_f)
typeof(eval_f) #integer임
as.integer(eval_f) #컴퓨터에선 1234의 factor로 나뉜걸 알 수 있음
class(eval_f) #class는 factor
plot(eval_f)
```
...
bond_rating_f # 알파벳순의로 순서를 매겨줌
plot(bond_rating_f)
```

### 리스트 {#financial-r-list}
...
cor_m <- cor(cbind(growth, unemp)) #이런 값도 넣을 수 있는 것이 장점

(economy_lst <- list(name=name, year=year, growth=growth, unemp=unemp, cor_m=cor_m))

class(economy_lst) #list

eco_cor <- economy_lst$cor_m
class(eco_cor) # 이런식으로 리스트에서 하나씩 뽑아서 사용도 가능
```

## Split-Apply-Combine 전략 {#financial-r-split-apply-combine}
...
## 기본개념 {#financial-r-basic-concept}
...
## Apply 가족 (샤프지수) {#financial-r-sharp}
...
삼성전자, SK 이노베이션, 현대자동차를 이영애 누님이 한창 달리던 카드대란을 뒤로하고 새롭게 출범한 노무현 정권이후 샤프지수를 
...
samsung_return <- periodReturn(`005930.KS`, period='yearly',subset='2003::')

getSymbols('096770.KS',src='yahoo', from = "2003-01-01", auto.assign = TRUE)
`096770.KS`$`096770.KS.Close` <- `096770.KS`$`096770.KS.Adjusted`
sk_return <- periodReturn(`096770.KS`, period='yearly',subset='2003::')

getSymbols('005380.KS',src='yahoo', from = "2003-01-01", auto.assign = TRUE)
`005380.KS`$`005380.KS.Close` <- `005380.KS`$`005380.KS.Adjusted`
hmc_return <- periodReturn(`005380.KS`, period='yearly',subset='2003::')

sharpe_ratio <- function(returns) {
  (mean(returns) - .0003) / sd(returns)
}

portfolio_return_lst <- list(samsung_return, sk_return, hmc_return)
...
## `lapply`, `sapply`, `vapply` {#financial-r-apply-family} 
...
# 함수형 프로그래밍 {#financial-r-purrr}

`purrr` 팩키지를 통해서 `lapply`, `sapply`, `vapply`등 다소 복잡하고 헤깔리는 함수형 프로그래밍 개념을 통합하였다.
`map()` 함수를 사용할 경우 깔끔하게 정리하는 것이 가능하다.

``` {r fe-map-purrr}
map(portfolio_return_lst, SharpeRatio.annualized, Rf=.0003, geometric=FALSE) 
```
